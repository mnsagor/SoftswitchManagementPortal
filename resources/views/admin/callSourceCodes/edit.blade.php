        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.callSourceCode.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.call-source-codes.update", [$callSourceCode->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('zone') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="zone_id" id="zone_id" required>
                                @foreach($zones as $id => $zone)
                                    <option value="{{ $id }}" {{ (old('zone_id') ? old('zone_id') : $callSourceCode->zone->id ?? '') == $id ? 'selected' : '' }}>{{ $zone }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('zone'))
