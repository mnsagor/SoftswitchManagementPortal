    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.employees.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Employee::SEX_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('sex') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="sex_{{ $key }}" name="sex" value="{{ $key }}" {{ old('sex', '1') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payroll_emp') ? 'is-invalid' : '' }}" type="text" name="payroll_emp" id="payroll_emp" value="{{ old('payroll_emp', '') }}">
