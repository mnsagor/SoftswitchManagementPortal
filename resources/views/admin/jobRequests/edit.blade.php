...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.jobRequest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.job-requests.update", [$jobRequest->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('network_type') ? 'is-invalid' : '' }}" name="network_type_id" id="network_type_id" required>
                    @foreach($network_types as $id => $network_type)
                        <option value="{{ $id }}" {{ (old('network_type_id') ? old('network_type_id') : $jobRequest->network_type->id ?? '') == $id ? 'selected' : '' }}>{{ $network_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job_type') ? 'is-invalid' : '' }}" name="job_type_id" id="job_type_id" required>
                    @foreach($job_types as $id => $job_type)
                        <option value="{{ $id }}" {{ (old('job_type_id') ? old('job_type_id') : $jobRequest->job_type->id ?? '') == $id ? 'selected' : '' }}>{{ $job_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('request_type') ? 'is-invalid' : '' }}" name="request_type_id" id="request_type_id" required>
                    @foreach($request_types as $id => $request_type)
                        <option value="{{ $id }}" {{ (old('request_type_id') ? old('request_type_id') : $jobRequest->request_type->id ?? '') == $id ? 'selected' : '' }}>{{ $request_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('request_status') ? 'is-invalid' : '' }}" name="request_status_id" id="request_status_id" required>
                    @foreach($request_statuses as $id => $request_status)
                        <option value="{{ $id }}" {{ (old('request_status_id') ? old('request_status_id') : $jobRequest->request_status->id ?? '') == $id ? 'selected' : '' }}>{{ $request_status }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number') ? 'is-invalid' : '' }}" type="text" name="number" id="number" value="{{ old('number', $jobRequest->number) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('agw_ip') ? 'is-invalid' : '' }}" type="text" name="agw_ip" id="agw_ip" value="{{ old('agw_ip', $jobRequest->agw_ip) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tid') ? 'is-invalid' : '' }}" type="text" name="tid" id="tid" value="{{ old('tid', $jobRequest->tid) }}" required>
                @if($errors->has('tid'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{!! old('note', $jobRequest->note) !!}</textarea>
                @if($errors->has('note'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('requested_by') ? 'is-invalid' : '' }}" name="requested_by_id" id="requested_by_id" required>
                    @foreach($requested_bies as $id => $requested_by)
                        <option value="{{ $id }}" {{ (old('requested_by_id') ? old('requested_by_id') : $jobRequest->requested_by->id ?? '') == $id ? 'selected' : '' }}>{{ $requested_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('request_time') ? 'is-invalid' : '' }}" type="text" name="request_time" id="request_time" value="{{ old('request_time', $jobRequest->request_time) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('verified_by') ? 'is-invalid' : '' }}" name="verified_by_id" id="verified_by_id">
                    @foreach($verified_bies as $id => $verified_by)
                        <option value="{{ $id }}" {{ (old('verified_by_id') ? old('verified_by_id') : $jobRequest->verified_by->id ?? '') == $id ? 'selected' : '' }}>{{ $verified_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('verification_time') ? 'is-invalid' : '' }}" type="text" name="verification_time" id="verification_time" value="{{ old('verification_time', $jobRequest->verification_time) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('approved_by') ? 'is-invalid' : '' }}" name="approved_by_id" id="approved_by_id">
                    @foreach($approved_bies as $id => $approved_by)
                        <option value="{{ $id }}" {{ (old('approved_by_id') ? old('approved_by_id') : $jobRequest->approved_by->id ?? '') == $id ? 'selected' : '' }}>{{ $approved_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('approval_time') ? 'is-invalid' : '' }}" type="text" name="approval_time" id="approval_time" value="{{ old('approval_time', $jobRequest->approval_time) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('approval_note') ? 'is-invalid' : '' }}" name="approval_note" id="approval_note">{!! old('approval_note', $jobRequest->approval_note) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('rejected_by') ? 'is-invalid' : '' }}" name="rejected_by_id" id="rejected_by_id">
                    @foreach($rejected_bies as $id => $rejected_by)
                        <option value="{{ $id }}" {{ (old('rejected_by_id') ? old('rejected_by_id') : $jobRequest->rejected_by->id ?? '') == $id ? 'selected' : '' }}>{{ $rejected_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('rejection_time') ? 'is-invalid' : '' }}" type="text" name="rejection_time" id="rejection_time" value="{{ old('rejection_time', $jobRequest->rejection_time) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('rejection_note') ? 'is-invalid' : '' }}" name="rejection_note" id="rejection_note">{!! old('rejection_note', $jobRequest->rejection_note) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('script') ? 'is-invalid' : '' }}" name="script" id="script">{{ old('script', $jobRequest->script) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_force_request') ? 'is-invalid' : '' }}" name="is_force_request" id="is_force_request">
                    ...
                    @foreach(App\Models\JobRequest::IS_FORCE_REQUEST_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_force_request', $jobRequest->is_force_request) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
