                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.job-types.update", [$jobType->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $jobType->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('is_active') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\JobType::IS_ACTIVE_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="is_active_{{ $key }}" name="is_active" value="{{ $key }}" {{ old('is_active', $jobType->is_active) === (string) $key ? 'checked' : '' }} required>
