...
@section('content')
@can('network_type_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.network-types.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.networkType.title_singular') }}
            </a>
            <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                {{ trans('global.app_csvImport') }}
            </button>
            @include('csvImport.modal', ['model' => 'NetworkType', 'route' => 'admin.network-types.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-NetworkType">
                    ...
                    <tr>
                        ...
                        <td>
                            <select class="search" strict="true">
                                ...
                                @foreach(App\Models\NetworkType::IS_ACTIVE_RADIO as $key => $item)
                ...
                <tbody>
                    @foreach($networkTypes as $key => $networkType)
                        <tr data-entry-id="{{ $networkType->id }}">
                            <td>
                            ...
                            <td>
                                @can('network_type_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.network-types.show', $networkType->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('network_type_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.network-types.edit', $networkType->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
