...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.oltJobRequest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.olt-job-requests.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('olt_ip') ? 'is-invalid' : '' }}" name="olt_ip_id" id="olt_ip_id" required>
                    @foreach($olt_ips as $id => $olt_ip)
                        <option value="{{ $id }}" {{ old('olt_ip_id') == $id ? 'selected' : '' }}>{{ $olt_ip }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('interface') ? 'is-invalid' : '' }}" type="text" name="interface" id="interface" value="{{ old('interface', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('service_type') ? 'is-invalid' : '' }}" name="service_type" id="service_type" required>
                    ...
                    @foreach(App\Models\OltJobRequest::SERVICE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('service_type', 'both') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('device_ip') ? 'is-invalid' : '' }}" type="text" name="device_ip" id="device_ip" value="{{ old('device_ip', '') }}">
