    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.oso-number-profiles.update", [$osoNumberProfile->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('oso_agw_ip') ? 'is-invalid' : '' }}" name="oso_agw_ip_id" id="oso_agw_ip_id" required>
                ...
                @if($errors->has('oso_agw_ip'))
                    <span class="text-danger">{{ $errors->first('oso_agw_ip') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\OsoNumberProfile::IS_ACTIVE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_active') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_active_{{ $key }}" name="is_active" value="{{ $key }}" {{ old('is_active', $osoNumberProfile->is_active) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\OsoNumberProfile::IS_TD_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_td') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_td_{{ $key }}" name="is_td" value="{{ $key }}" {{ old('is_td', $osoNumberProfile->is_td) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\OsoNumberProfile::IS_ISD_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_isd') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_isd_{{ $key }}" name="is_isd" value="{{ $key }}" {{ old('is_isd', $osoNumberProfile->is_isd) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\OsoNumberProfile::IS_EISD_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_eisd') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_eisd_{{ $key }}" name="is_eisd" value="{{ $key }}" {{ old('is_eisd', $osoNumberProfile->is_eisd) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\OsoNumberProfile::IS_PBX_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_pbx') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_pbx_{{ $key }}" name="is_pbx" value="{{ $key }}" {{ old('is_pbx', $osoNumberProfile->is_pbx) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pbx_poilot_number') ? 'is-invalid' : '' }}" type="text" name="pbx_poilot_number" id="pbx_poilot_number" value="{{ old('pbx_poilot_number', $osoNumberProfile->pbx_poilot_number) }}">
