...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tndpImsOltProfile.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tndp-ims-olt-profiles.update", [$tndpImsOltProfile->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('olt_name') ? 'is-invalid' : '' }}" name="olt_name_id" id="olt_name_id" required>
                    @foreach($olt_names as $id => $olt_name)
                        <option value="{{ $id }}" {{ (old('olt_name_id') ? old('olt_name_id') : $tndpImsOltProfile->olt_name->id ?? '') == $id ? 'selected' : '' }}>{{ $olt_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $tndpImsOltProfile->date) }}" required>
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job_type') ? 'is-invalid' : '' }}" name="job_type_id" id="job_type_id" required>
                    @foreach($job_types as $id => $job_type)
                        <option value="{{ $id }}" {{ (old('job_type_id') ? old('job_type_id') : $tndpImsOltProfile->job_type->id ?? '') == $id ? 'selected' : '' }}>{{ $job_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('device_type') ? 'is-invalid' : '' }}" name="device_type" id="device_type" required>
                    ...
                    @foreach(App\Models\TndpImsOltProfile::DEVICE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('device_type', $tndpImsOltProfile->device_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('no_of_ports') ? 'is-invalid' : '' }}" name="no_of_ports" id="no_of_ports">
                    ...
                    @foreach(App\Models\TndpImsOltProfile::NO_OF_PORTS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('no_of_ports', $tndpImsOltProfile->no_of_ports) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('serial_number') ? 'is-invalid' : '' }}" type="text" name="serial_number" id="serial_number" value="{{ old('serial_number', $tndpImsOltProfile->serial_number) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('interface') ? 'is-invalid' : '' }}" type="text" name="interface" id="interface" value="{{ old('interface', $tndpImsOltProfile->interface) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ip') ? 'is-invalid' : '' }}" type="text" name="ip" id="ip" value="{{ old('ip', $tndpImsOltProfile->ip) }}" required>
                @if($errors->has('ip'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number') ? 'is-invalid' : '' }}" type="text" name="number" id="number" value="{{ old('number', $tndpImsOltProfile->number) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('port_number') ? 'is-invalid' : '' }}" type="text" name="port_number" id="port_number" value="{{ old('port_number', $tndpImsOltProfile->port_number) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('service') ? 'is-invalid' : '' }}" name="service" id="service" required>
                    ...
                    @foreach(App\Models\TndpImsOltProfile::SERVICE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('service', $tndpImsOltProfile->service) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id">
                    @foreach($statuses as $id => $status)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $tndpImsOltProfile->status->id ?? '') == $id ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $tndpImsOltProfile->description) !!}</textarea>
