        ...
        <div class="col-md-12">
            @can('region_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.regions.create') }}">
                            ...
                            <tbody>
                                @foreach($regions as $key => $region)
                                    <tr data-entry-id="{{ $region->id }}">
                                        ...
                                        <td>
                                            @can('region_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.regions.show', $region->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('region_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.regions.edit', $region->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('region_delete')
                                                <form action="{{ route('frontend.regions.destroy', $region->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('region_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.regions.massDestroy') }}",
